...
const ASSETS = {
  ...
  "USDJPY": { multiplier: 100000, price: 150.00, spread: 0.01 },
  "XAUUSD": { multiplier: 100, price: 2030.00, spread: 0.20 },
  "US30": { multiplier: 1, price: 38000.00, spread: 2.0 },
  "NQ": { multiplier: 1, price: 17500.00, spread: 1.25 }
};
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (user.user) {
      ...
      const { error: settingsError } = await supabase
       ...
       if (settingsError) {
           console.warn(`Warning: Could not create settings for ${email} (might already exist via trigger?):`, settingsError.message);
       }
      ...
      for (let t = 0; t < 50; t++) {
        ...
        const exitDate = new Date(entryDate.getTime() + faker.number.int({ min: 1000 * 60 * 5, max: 1000 * 60 * 60 * 24 })); // 5 mins to 1 day duration
      ...
      if (tradesError) {
        ...
        await supabase.from('accounts').update({ current_balance: currentBalance }).eq('id', account.id);
      }
    }
  }
