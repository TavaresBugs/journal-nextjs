...
import { sharePlaybook, unsharePlaybook, getPublicPlaybooks, getMySharedPlaybooks, togglePlaybookStar } from "@/services/community/playbook";
...
describe("Playbook Service", () => {
  ...
  describe("sharePlaybook", () => {
    it("should return null if user is not authenticated", async () => {
      vi.mocked(mocks.supabase.auth.getUser).mockResolvedValue({ data: { user: null }, error: null } as any);
    ...
    it("should create new share if not existing", async () => {
      vi.mocked(mocks.supabase.auth.getUser).mockResolvedValue({ data: { user: { id: "u1" } }, error: null } as any);
    ...
    it("should update existing share to public", async () => {
      ...
      vi.mocked(mocks.supabase.from).mockReturnValueOnce({
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        maybeSingle: vi.fn().mockResolvedValue({ data: { id: "share1", is_public: false }, error: null }),
      } as any);
  ...
  describe("getPublicPlaybooks", () => {
      ...
      it("should return list of playbooks", async () => {
        vi.mocked(mocks.supabase.auth.getUser).mockResolvedValue({ data: { user: { id: 'u1' } } } as any);
  ...
  describe("togglePlaybookStar", () => {
      it("should call rpc", async () => {
          ...
          expect(result).toBe(true);
          expect(mocks.supabase.rpc).toHaveBeenCalledWith("toggle_playbook_star", { p_shared_playbook_id: "share1" });
      });
  });
});
