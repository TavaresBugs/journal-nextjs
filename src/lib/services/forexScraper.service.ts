...
export async function scrapeForexFactory(): Promise<ScrapedEvent[]> {
  ...
  try {
    ...
    const rawEvents = await page.evaluate(() => {
      ...
      // Armazena a data corrente para preencher linhas vazias (comum no FF)
      let currentDateText = '' 
      ...
      rows.forEach((row) => {
        ...
        let dateText = row.querySelector('.calendar__date .date')?.textContent?.trim() || 
        ...
        let impact = 'none'
        if (impactTitle.includes('High') || impactClass.includes('icon--ff-impact-red')) impact = 'high'
        else if (impactTitle.includes('Medium') || impactClass.includes('icon--ff-impact-ora')) impact = 'medium'
        else if (impactTitle.includes('Low') || impactClass.includes('icon--ff-impact-yel')) impact = 'low'
    ...
    for (const raw of rawEvents) {
      ...
      // Converter tempo para 24h
      const time24 = convertTo24Hour(raw.time)

      // Só adicionar se tiver tempo válido (filtrar supostos eventos vazios)
      if (isoDate && time24) {
        events.push({
          date: isoDate,
          time: time24,
