 ...
 */
export async function getEventsByDateRange(startDate: Date, endDate: Date): Promise<EconomicEvent[]> {
  const startStr = startDate.toISOString().split('T')[0]
  const endStr = endDate.toISOString().split('T')[0]
 ...
 */
export async function upsertEconomicEvents(events: DBEvent[]): Promise<{ success: boolean; count: number }> {
  if (events.length === 0) {
