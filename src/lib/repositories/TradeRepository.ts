...
export class TradeRepository extends BaseRepository<DBTrade> {
  ...
  async getByIdDomain(tradeId: string, userId?: string): Promise<Result<Trade, AppError>> {
    ...
    if (userId && result.data.user_id !== userId) {
      this.logger.warn('Unauthorized trade access attempt', { tradeId, userId, ownerId: result.data.user_id });
      return { 
        data: null, 
  ...
  async getByIdWithAuth(tradeId: string, userId: string): Promise<Result<Trade, AppError>> {
    ...
    if (result.error) {
      if (result.error.code === ErrorCode.DB_NOT_FOUND || result.error.code === ErrorCode.AUTH_FORBIDDEN) {
   ...
   * }
   */
  async createWithJournal(trade: Trade, journalId: string, userId: string): Promise<Result<Trade, AppError>> {
    const startTime = performance.now();
    ...
    try {
      ...
      if (journal.user_id !== userId) {
        this.logger.warn('Unauthorized attempt to add trade to journal', { journalId, userId, ownerId: journal.user_id });
        return {
          data: null,
          error: new AppError('Not authorized to add trades to this journal', ErrorCode.AUTH_FORBIDDEN, 403)
        };
      }
      ...
      if (tradeError || !createdTrade) {
        ...
        return {
          data: null,
          error: new AppError(tradeError?.message || 'Failed to create trade', ErrorCode.DB_QUERY_FAILED, 500)
        };
      }
      ...
      if (junctionError) {
        ...
        this.logger.error('Failed to create junction entry, rolling back trade', { error: junctionError });
        await this.supabase.from(this.tableName).delete().eq('id', createdTrade.id);
        
        return {
          data: null,
          error: new AppError('Failed to associate trade with journal', ErrorCode.DB_QUERY_FAILED, 500)
        };
      }
  ...
  async getByAccountId(accountId: string, options?: QueryOptions): Promise<Result<Trade[], AppError>> {
    ...
    const durationMs = performance.now() - startTime;
    this.logSlowQuery('getByAccountId', durationMs, { accountId, resultCount: result.data?.length });
  ...
  async createDomain(trade: Trade): Promise<Result<Trade, AppError>> {
    ...
    this.logger.info('Trade created successfully', { tradeId: createdTrade?.id, durationMs: Math.round(durationMs) });
