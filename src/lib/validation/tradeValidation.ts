...
import { REQUIRED_FIELDS, ERROR_MESSAGES, FIELD_LABELS } from "./tradeValidation.types";
 ...
 * - entryDate
 */
export function validateRequiredFields(input: TradeValidationInput): ValidationError[] {
  const errors: ValidationError[] = [];
...
export function validateTrade(
  ...
  // 2. Date validation
  allErrors.push(
    ...validateDates(input.entryDate, input.entryTime, input.exitDate, input.exitTime, config)
  );
...
export { REQUIRED_FIELDS, ERROR_MESSAGES, FIELD_LABELS } from "./tradeValidation.types";
