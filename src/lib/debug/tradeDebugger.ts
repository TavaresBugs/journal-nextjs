...
export const TradeDebugger = {
  ...
  log(trade: TradeData, source: string = "Unknown") {
    ...
    console.log(`%c🔍 Trade Debugger %c[${source}]`, styles.title, styles.muted);

    // ID Info
    console.log(`%cID:%c ${trade?.id}`, styles.key, styles.value);
    console.log(
    ...
    // User Info
    console.log(`%cUser ID:%c ${trade?.user_id}`, styles.key, styles.value);

    // Trade Info
    console.log(`%cStrategy:%c ${trade?.strategy}`, styles.key, styles.value);
    console.log(`%cOutcome:%c ${trade?.outcome}`, styles.key, styles.value);

    // Timestamps
    console.log(`%cCreated:%c ${trade?.created_at}`, styles.key, styles.value);
  ...
  compare(oldTrade: TradeData | null, newTrade: TradeData | null) {
    ...
    const keys = new Set([...Object.keys(oldTrade), ...Object.keys(newTrade)]);
  ...
  validate(
    trade: TradeData,
    requiredFields: string[] = ["id", "user_id", "strategy", "outcome"]
  ): boolean {
    if (typeof window === "undefined") return true;
    if (process.env.NODE_ENV !== "development") return true;

    const missingFields = requiredFields.filter(
      (field) => !(field in trade) || trade[field] === undefined
    );

    if (missingFields.length > 0) {
      console.warn(
        `%c⚠️ Trade Validation Failed - Missing fields: ${missingFields.join(", ")}`,
        styles.error
      );
      return false;
    }
