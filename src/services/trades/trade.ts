...
export async function getTrades(accountId: string): Promise<Trade[]> {
  ...
  if (!userId) {
    handleServiceError(new Error("User not authenticated"), "tradeService.getTrades", {
      severity: "silent",
    });
    return [];
  }
...
export async function getTradeHistoryLite(accountId: string): Promise<TradeLite[]> {
  ...
  const { data, error } = await supabase
    .from("trades")
    .select(
      "id, entry_date, entry_time, exit_date, exit_time, pnl, outcome, account_id, symbol, type, entry_price, exit_price, stop_loss, take_profit, lot, tags, strategy, setup, tf_analise, tf_entrada, market_condition, entry_quality, market_condition_v2, pd_array, session, user_id, commission, swap"
    )
    .eq("account_id", accountId)
    .eq("user_id", userId)
    .order("entry_date", { ascending: false })
...
export async function saveTrade(trade: Trade): Promise<boolean> {
  const userId = await getCurrentUserId();
  if (!userId) {
    handleServiceError(new Error("User not authenticated"), "tradeService.saveTrade", {
      severity: "silent",
    });
    return false;
  }
...
export async function deleteTrade(id: string): Promise<boolean> {
  const userId = await getCurrentUserId();
  if (!userId) {
    handleServiceError(new Error("User not authenticated"), "tradeService.deleteTrade", {
      severity: "silent",
    });
    return false;
  }
