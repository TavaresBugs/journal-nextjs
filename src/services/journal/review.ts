...
export async function getReviewsForContext(
  ...
  let query = supabase.from("mentor_reviews").select("*").eq("mentee_id", user.id);

  // Filter by IDs if provided
  const conditions = [];
  if (tradeIds.length > 0) conditions.push(`trade_id.in.(${tradeIds.join(",")})`);
  if (journalEntryIds.length > 0)
    conditions.push(`journal_entry_id.in.(${journalEntryIds.join(",")})`);
...
export async function markReviewAsRead(id: string): Promise<boolean> {
  const { error } = await supabase.from("mentor_reviews").update({ is_read: true }).eq("id", id);
