...
export const mapJournalEntryFromDB = (db: DBJournalEntry): JournalEntry => ({
  ...
  tradeIds:
    db.journal_entry_trades?.map((jet: DBJournalEntryTrade) => jet.trade_id) ||
    (db.trade_id ? [db.trade_id] : []),
  images: db.journal_images
    ? db.journal_images.map((img: DBJournalImage) => ({
        id: img.id,
        userId: img.user_id,
 ...
 */
export const mapJournalEntryToDB = (
  app: JournalEntry
...
export async function getJournalEntries(accountId: string): Promise<JournalEntry[]> {
  ...
  if (!userId) {
    handleServiceError(new Error("User not authenticated"), "journalService.getJournalEntries", {
      severity: "silent",
    });
    return [];
  }
...
export async function saveJournalEntry(entry: JournalEntry): Promise<boolean> {
  const userId = await getCurrentUserId();
  if (!userId) {
    handleServiceError(new Error("User not authenticated"), "journalService.saveJournalEntry", {
      severity: "silent",
    });
    return false;
  }
  ...
  if (entry.images) {
        ...
        for (let i = 0; i < base64Array.length; i++) {
          ...
          if (typeof base64 === "string" && base64.startsWith("data:image")) {
            try {
              ...
              if (webpError) {
                handleServiceError(webpError, `journalService.uploadWebP.${timeframe}`, {
                  severity: "warn",
                });
              }
              ...
              } = supabase.storage.from("journal-images").getPublicUrl(webpFileName);
              ...
              if (process.env.NODE_ENV === "development") {
                ...
                console.log(
                  `[Journal] Image ${timeframe}-${i}: ${(compressed.originalSize / 1024).toFixed(0)}KB → ${(compressed.compressedSizeWebP / 1024).toFixed(0)}KB (-${savings}%)`
                );
              }
            } catch (err) {
              handleServiceError(err, `journalService.processImage.${timeframe}`, {
                severity: "warn",
              });
            }
      ...
      try {
        ...
        if (!dbError && dbImages) {
          ...
          if (filesToDelete.length > 0) {
            ...
            await supabase.storage.from("journal-images").remove(filesToDelete);
          }
        }
      } catch (cleanupError) {
        handleServiceError(cleanupError, "journalService.cleanupOrphanedImages", {
          severity: "warn",
        });
      }
    }
    ...
    if (imagesToSave.length > 0) {
      const { error: imagesError } = await supabase.from("journal_images").insert(imagesToSave);
  ...
  if (entry.tradeIds && entry.tradeIds.length > 0) {
    ...
    if (deleteTradesError) {
      handleServiceError(deleteTradesError, "journalService.deleteTradeLinks", {
        severity: "warn",
      });
    }
...
export async function deleteJournalEntry(id: string): Promise<boolean> {
  const userId = await getCurrentUserId();
  if (!userId) {
    handleServiceError(new Error("User not authenticated"), "journalService.deleteJournalEntry", {
      severity: "silent",
    });
    return false;
  }
