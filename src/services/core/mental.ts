 ...
 */
export async function updateMentalEntry(id: string, data: Partial<CreateMentalEntryInput>): Promise<void> {
    const { error } = await supabase
        .from('mental_entries')
        .update({
            trigger_event: data.triggerEvent,
 ...
 */
export async function getZoneStats(limit = 30): Promise<{ aGame: number; bGame: number; cGame: number }> {
    const entries = await getMentalEntries(limit);
