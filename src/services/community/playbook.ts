...
import { SharedPlaybook, Playbook } from "@/types";
...
export async function getPublicPlaybooks(limit = 20, offset = 0): Promise<SharedPlaybook[]> {
  ...
  if (playbookPairs.length > 0) {
    ...
    for (const pair of playbookPairs) {
      ...
      let currentStreak = 0;
      let maxStreak = 0;
      const sortedTrades = [...typedTrades].sort(
      ...
      const avgDurationMinutes =
        validDurationTrades > 0 ? totalDurationMinutes / validDurationTrades : 0;
      const avgDuration =
