...
describe('exportService', () => {
    ...
    describe('exportAllData', () => {
        ...
        it('should export all data correctly', async () => {
            ...
            const mockAccounts = [{ id: 'acc-1', user_id: userId, name: 'Test Account', initial_balance: 1000, current_balance: 1000 }];
            const mockTrades = [{ id: 'trade-1', user_id: userId, account_id: 'acc-1', symbol: 'EURUSD', entry_price: 1.1, exit_price: 1.2, pnl: 100 }];
            const mockEntries = [{ id: 'entry-1', user_id: userId, trade_id: 'trade-1', title: 'My Trade' }];
            ...
            const mockImages = [{ id: 'img-1', user_id: userId, journal_entry_id: 'entry-1', url: 'http://example.com/img.png' }];
            ...
            const createQueryMock = (data: any[]) => {
                ...
                const eqMock = vi.fn().mockReturnValue({ range: rangeMock, maybeSingle: vi.fn().mockReturnValue({ data: data[0] || null, error: null }) });
            ...
            (supabase.from as any).mockImplementation((tableName: string) => {
                switch (tableName) {
                    case 'accounts': return createQueryMock(mockAccounts);
                    case 'trades': return createQueryMock(mockTrades);
                    case 'journal_entries': return createQueryMock(mockEntries);
                    case 'playbooks': return createQueryMock(mockPlaybooks);
                    case 'daily_routines': return createQueryMock(mockRoutines);
                    case 'settings': return createQueryMock([mockSettings]);
                    case 'journal_images': return createQueryMock(mockImages);
