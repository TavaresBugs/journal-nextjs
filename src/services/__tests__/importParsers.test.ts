...
import { detectColumnMapping, transformTrades, ColumnMapping } from '../importParsers';
import { RawTradeData } from '../importService';

describe('importParsers', () => {
        it('detects standard MetaTrader English headers', () => {
            const headers = ['Open Time', 'Type', 'Size', 'Symbol', 'Open Price', 'S / L', 'T / P', 'Close Time', 'Close Price', 'Commission', 'Taxes', 'Swap', 'Profit'];
        ...
        it('detects Portuguese headers', () => {
            const headers = ['Data Abertura', 'Tipo', 'Volume', 'Ativo', 'Preço Entrada', 'Data Fechamento', 'Preço Saída', 'Lucro', 'Comissão', 'Taxas'];
            const mapping = detectColumnMapping(headers);
    ...
    describe('transformTrades', () => {
        ...
        it('converts timezone correctly (UTC to NY)', () => {
             // 10:00 UTC is 06:00 NY (EDT) or 05:00 NY (EST). Oct 27 is EDT (UTC-4).
             // 10:00 UTC - 4 = 06:00 NY
             const trades = transformTrades(
                mockRawData,
                mockMapping,
                'metatrader',
             ...
             const trade = trades[0];
             expect(trade.entryTime).toMatch(/^06:00:00$/);
        });
