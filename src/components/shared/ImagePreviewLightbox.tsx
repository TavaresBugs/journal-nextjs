...
export function ImagePreviewLightbox({
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-200 flex items-center justify-center bg-black/90 backdrop-blur-sm transition-opacity duration-300"
      onClick={onClose}
    >
      {/* Close Button */}
      <IconActionButton
      ...
      {/* Image Label */}
      <div className="pointer-events-none absolute top-4 left-1/2 z-50 flex -translate-x-1/2 gap-2 rounded-full bg-black/70 px-4 py-2 text-sm font-medium text-cyan-400">
      ...
      {images.length > 1 && (
        <>
          <IconActionButton
            variant="back"
            size="lg"
            className="absolute top-1/2 left-4 z-50 -translate-y-1/2 text-gray-200"
            onClick={handlePrev}
          />
          <IconActionButton
            variant="next"
            size="lg"
            className="absolute top-1/2 right-4 z-50 -translate-y-1/2 text-gray-200"
            onClick={handleNext}
          />
        </>
      )}
          ...
          {({ zoomIn, zoomOut, resetTransform, instance }) => {
            ...
            return (
              <>
                ...
                {/* Zoom Controls */}
                <div className="absolute bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full bg-black/70 px-4 py-2 backdrop-blur-sm">
                  {/* Reset Button */}
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => resetTransform()}
                    className="h-8 w-8 rounded-full text-white transition-colors hover:bg-white/10"
                  ...
                  <span
                    ...
                    className="min-w-14 text-center font-mono text-sm text-white"
                  >
