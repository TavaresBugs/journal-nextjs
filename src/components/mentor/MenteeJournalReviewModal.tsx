...
export function MenteeJournalReviewModal({
  ...
  const [addingType, setAddingType] = useState<"comment" | "suggestion" | "correction" | null>(
    null
  );
  ...
  const renderReviewItem = (r: MentorReview, colorClass: string) => {
    ...
    return (
      <div
        key={r.id}
        className={`border-b text-sm text-gray-300 ${colorClass} pb-2 last:border-0 last:pb-0`}
      >
        ...
        {isEditing ? (
          <div className="mt-2 space-y-2">
            <textarea
              ...
              className="w-full rounded border border-gray-700 bg-gray-900 p-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none"
              rows={3}
            />
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="📝 Revisão do Mentor" maxWidth="full">
      <div className="grid grid-cols-1 gap-6 pb-20 lg:grid-cols-2">
        ...
        <div className="border-gray-800 pr-2 lg:border-r">
          <JournalEntryContent entry={entry} linkedTrades={trade ? [trade] : []} />
        </div>

        {/* Right Side: Structured Review Panel */}
        <div className="flex min-h-[500px] flex-col rounded-lg border border-gray-800 bg-gray-900/30 p-6">
          <div className="mb-6 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-xl font-bold text-gray-100">
              <span>💬</span> Painel de Feedback
            </h3>
            <div className="flex gap-2">
              <span className="rounded border border-gray-700 bg-gray-800 px-2 py-1 text-xs text-gray-400">
          ...
          <div className="mb-8 space-y-6">
            {isLoadingReviews && (
              <div className="flex justify-center py-4">
                <span className="animate-pulse text-sm text-cyan-400">Carregando feedback...</span>
              </div>
            )}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between border-b border-green-500/20 pb-1">
                ...
                <Button
                  onClick={() => {
                    setAddingType("comment");
                    setReviewContent("");
                  }}
                  size="sm"
                  variant="ghost"
                  className="h-6 px-2 text-xs text-green-400 hover:bg-green-500/10 hover:text-green-300"
                >
                  + Adicionar
                </Button>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between border-b border-yellow-500/20 pb-1">
                <h4 className="text-sm font-bold tracking-wider text-yellow-400 uppercase">
                  Sugestões / Melhorias
                </h4>
                <Button
                  onClick={() => {
                    setAddingType("suggestion");
                    setReviewContent("");
                  }}
                  size="sm"
                  variant="ghost"
                  className="h-6 px-2 text-xs text-yellow-400 hover:bg-yellow-500/10 hover:text-yellow-300"
                >
                  + Adicionar
                </Button>
              </div>
              <div className="min-h-[80px] space-y-3 rounded-lg border border-yellow-500/10 bg-yellow-500/5 p-3">
                {suggestions.length > 0 ? (
                  ...
                  <span className="text-sm text-gray-500 italic">Nenhuma sugestão registrada.</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between border-b border-red-500/20 pb-1">
                <h4 className="text-sm font-bold tracking-wider text-red-400 uppercase">
                  Erros / Correções
                </h4>
                <Button
                  onClick={() => {
                    setAddingType("correction");
                    setReviewContent("");
                  }}
                  size="sm"
                  variant="ghost"
              ...
              <div className="min-h-[80px] space-y-3 rounded-lg border border-red-500/10 bg-red-500/5 p-3">
                {corrections.length > 0 ? (
                  ...
                  <span className="text-sm text-gray-500 italic">Nenhuma correção registrada.</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 border-t border-gray-800 pt-4">
            {addingType && (
              <div className="animate-in fade-in slide-in-from-bottom-2 sticky bottom-4 z-10 space-y-3 rounded-lg border border-gray-800 bg-gray-950/50 p-4 shadow-xl">
                <div className="flex items-center justify-between">
                  <span
                    className={`text-sm font-bold uppercase ${
                      addingType === "correction"
                        ? "text-red-400"
                        : addingType === "suggestion"
                          ? "text-yellow-400"
                          : "text-green-400"
                    }`}
                  >
                ...
                <textarea
                  ...
                  autoFocus
                  className={`h-32 w-full resize-none rounded-lg border bg-gray-900 p-3 text-sm text-gray-200 focus:outline-none ${
                    addingType === "correction"
