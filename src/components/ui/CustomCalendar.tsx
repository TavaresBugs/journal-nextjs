...
export function CustomCalendar({ selected, onSelect, onClose }: CalendarProps) {
  ...
  return (
    <div className="w-[320px] rounded-xl border border-gray-700 bg-gray-800 p-4 shadow-2xl">
      ...
      <div className="grid grid-cols-7 gap-y-1">
        {calendarDays.map((item, index) => (
          <button
            ...
            className={`mx-auto flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium transition-colors focus:outline-none ${
              !item.isCurrentMonth
                ? "cursor-default text-gray-600"
                : isSelected(item.day)
                  ? "bg-cyan-500 font-bold text-gray-900"
                  : isToday(item.day)
                    ? "text-gray-100 ring-2 ring-cyan-500"
