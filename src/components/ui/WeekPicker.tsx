...
export function WeekPickerCalendar({
  ...
  return (
    <div className="w-[360px] rounded-xl border border-gray-700 bg-gray-800 p-4 shadow-2xl">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-gray-700 hover:text-cyan-400 focus:outline-none"
          >
            <svg
              width="16"
              height="16"
      ...
      <div className="mb-2 grid grid-cols-8">
        <div className="flex h-8 items-center justify-center text-xs font-semibold text-gray-500">
          Sem
        </div>
        {WEEKDAYS_PT.map((day) => (
          <div
            key={day}
            className="flex h-8 items-center justify-center text-xs font-semibold text-gray-500 uppercase"
          >
            {day}
          </div>
        ))}
        ...
        {weeks.map((week, weekIndex) => {
          ...
          return (
            <div
              ...
              className={`grid cursor-pointer grid-cols-8 rounded-lg transition-all ${
                isSelected
            ...
            >
              {/* Week Number */}
              <div
                className={`flex h-10 items-center justify-center rounded-l-lg text-sm font-bold ${isSelected ? "bg-cyan-500 text-gray-900" : "text-cyan-400"} `}
              >
                {weekNum}
              </div>
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <button
        ...
        className="flex items-center gap-3 rounded-lg border border-gray-700 bg-gray-800/50 px-4 py-2 text-white transition-colors hover:border-cyan-500/50 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500"
      >
        <span className="font-medium">
          Semana {weekNumber}, {selectedWeekStart.getFullYear()}
        </span>
        <svg
      ...
      <span className="ml-3 text-sm text-gray-400">
        {format(selectedWeekStart, "dd/MM", { locale: ptBR })} -{" "}
        {format(selectedWeekEnd, "dd/MM", { locale: ptBR })}
      </span>
