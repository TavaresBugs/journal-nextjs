  ...
  if (trades.length === 0) {
    return (
      <GlassCard className="flex min-h-[400px] flex-col items-center justify-center p-8">
        ...
        <div className="text-gray-500">Nenhum trade nos últimos 30 dias</div>
      </GlassCard>
    );
  }
  ...
  return (
    <GlassCard className="p-8">
      <div className="mb-8 flex items-center justify-between">
        <h3 className="text-base font-medium text-gray-400">Timeline de Performance</h3>
        <span className="text-sm text-gray-500">
          {chartData.length} dia{chartData.length !== 1 ? "s" : ""} com trades
        </span>
      </div>

      <LightweightChartWrapper height={400} onChartReady={onChartReady} />
    </GlassCard>
  );
}
