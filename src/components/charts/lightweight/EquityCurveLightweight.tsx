...
export function EquityCurveLightweight({
  ...
  const chartData = useMemo(() => {
    ...
    const startDate =
      firstTradeDate && new Date(firstTradeDate) < new Date(accountDate)
        ? firstTradeDate
        : accountDate;
  ...
  const isProfit =
    chartData.length > 0 ? chartData[chartData.length - 1].value >= initialBalance : true;
  ...
  return (
    <GlassCard className="p-8">
      <div className="mb-8 flex items-center justify-between">
        <h3 className="text-base font-medium text-gray-400">Curva de Capital</h3>
        <div className="flex items-center gap-6 text-sm">
          ...
          <div className="flex items-center gap-2">
            <div
              className={`h-2 w-2 rounded-full ${isProfit ? "bg-zorin-accent" : "bg-red-400"}`}
            ></div>
      ...
      <LightweightChartWrapper height={450} onChartReady={onChartReady} />
    </GlassCard>
  );
}
