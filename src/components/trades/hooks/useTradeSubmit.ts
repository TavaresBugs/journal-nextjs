...
export function useTradeSubmit(options: UseTradeSubmitOptions): UseTradeSubmitReturn {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent, state: TradeFormState, computed: TradeFormComputedValues) => {
      e.preventDefault();
      ...
      try {
        ...
        const tradeData: Omit<Trade, "id" | "createdAt" | "updatedAt"> = {
          ...
          htfAligned:
            state.tfAnalise && state.tfEntrada ? !computed.alignmentResult.isWarning : undefined,
          rMultiple: undefined,
          pnl: undefined,
          outcome: "pending",
