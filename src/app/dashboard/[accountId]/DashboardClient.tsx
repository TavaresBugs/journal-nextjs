...
// Core UI Components (lightweight, always needed)
import {
  Card,
  CardHeader,
  CardTitle,
...
// Static imports for critical path (LCP elements)
import { DashboardHeader } from "@/components/dashboard/DashboardHeader";
import { DashboardMetrics } from "@/components/dashboard/DashboardMetrics";

// Skeletons (lightweight, needed for loading states)
import {
...
// from ~2MB to ~500KB, eliminating the 7-second Long Task
// ============================================

// Tab Components (loaded when user navigates to tab)
const DashboardJournal = dynamic(
  () => import("@/components/dashboard/tabs/DashboardJournal").then((mod) => mod.DashboardJournal),
  { loading: () => <DashboardContentSkeleton />, ssr: false }
);

const DashboardPlaybooks = dynamic(
  () =>
    import("@/components/dashboard/tabs/DashboardPlaybooks").then((mod) => mod.DashboardPlaybooks),
  { loading: () => <DashboardContentSkeleton />, ssr: false }
);

const DashboardOverview = dynamic(
  () =>
    import("@/components/dashboard/tabs/DashboardOverview").then((mod) => mod.DashboardOverview),
...
// Trade Components (loaded when interacting)
const TradeForm = dynamic(
  () => import("@/components/trades/TradeForm").then((mod) => mod.TradeForm),
  { loading: () => <FormSkeleton />, ssr: false }
);

const TradeCalendar = dynamic(
  () => import("@/components/trades/TradeCalendar").then((mod) => mod.TradeCalendar),
  { loading: () => <CalendarSkeleton />, ssr: false }
);

// Modal Components (loaded when user opens modal)
const DashboardModals = dynamic(
  () => import("@/components/dashboard/DashboardModals").then((mod) => mod.DashboardModals),
  { ssr: false }
);

const JournalEntryModal = dynamic(
  () => import("@/components/journal/JournalEntryModal").then((mod) => mod.JournalEntryModal),
  { ssr: false }
);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div className="relative z-10">
        {/* Header - Renders immediately with prefetched account (LCP element) */}
              ...
              <TabPanel value="relatorios" activeTab={activeTab}>
                {!data.loadingPhases.heavy.reports ? (
                  ...
                  <DashboardOverview
                    ...
                    accountCreatedAt={String(currentAccount.createdAt)}
