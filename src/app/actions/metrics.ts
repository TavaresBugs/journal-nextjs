...
export async function getAccountMetricsAction(accountId: string): Promise<AccountMetrics | null> {
  try {
    ...
    return {
      ...
      avgPnl: Number(metrics.avg_pnl ?? 0),
      pnlStdDev: Number(metrics.pnl_std_dev ?? 0),
      maxWinStreak: metrics.max_win_streak ?? 0,
      maxLossStreak: metrics.max_loss_streak ?? 0,
      currentStreak: metrics.current_streak ?? 0,
      currentStreakType: (metrics.current_streak_type as "win" | "loss" | "none") ?? "none",
      ...
      updatedAt: metrics.updated_at?.toISOString() ?? new Date().toISOString(),
    };
  } catch (error) {
