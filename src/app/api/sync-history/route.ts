...
export async function POST() {
  ...
  try {
    ...
    console.log(`[Sync History] Iniciando sync global de ${format(currentIterDate, 'MMM/yyyy')} até ${format(stopDate, 'MMM/yyyy')}`)

    while (isBefore(currentIterDate, stopDate) || currentIterDate.getMonth() === stopDate.getMonth()) {
    ...
    return NextResponse.json(
      { error: 'Falha na sincronização histórica' },
      { status: 500 }
    )
  }
}
