...
export function useMentalHub(): UseMentalHubReturn {
     ...
     */
    const saveEntry = useCallback(async (data: MentalEntryInput): Promise<boolean> => {
        try {
            setError(null);
     ...
     */
    const seedProfiles = useCallback(async (category: 'fear' | 'greed' | 'tilt' | 'all'): Promise<boolean> => {
        try {
            setIsLoading(true);
            setError(null);
