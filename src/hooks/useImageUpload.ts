...
export function useImageUpload(
  ...
  const cacheImage = useCallback((timeframe: string, base64: string, index: number) => {
  ...
  const processImage = useCallback(async (base64: string): Promise<string> => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (enableCache && keysToClean.size > 0) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`[useImageUpload] Cleanup: cleared ${clearedCount} cached images for context '${ctx}'`);
        }
      }
    };
